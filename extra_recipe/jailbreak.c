...
int jb_go(int action) {
    ...
    return 42;
#else
    extern int unjail(int);
    return kernel_base ? unjail(action) : -1;
#endif
}
...
unjail(int action)
{
    ...
    extern int unjail2(uint64_t, uint64_t, int);
    rv = unjail2(allproc + kaslr_shift, kern_cred, action);
    
    kwrite_uint64(our_proc + offsetof_p_ucred, our_cred);
    
    extern int unjail3(int);
    rv = unjail3(action);
