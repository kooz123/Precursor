...
// Patch Sandbox
static void
nukesb(int nukesb, int uref, uint64_t kernbase, uint64_t slide)
{
    checkvad();
...
remappage[remapcnt++] = (x & (~PMK));\
    ...
    RemapPage(release);
    if (NewPointer(release) == (NewPointer(release+13) - 13)) {
        copyout(NewPointer(release), "CydeloadedARM", 13);
    }
    {
...
static int
IOConnectTrap4(int conn, long unused, uint64_t x1, uint64_t x2, uint64_t x0, uint64_t func)
{
    uint32_t rv;
...
unjail3(int action)
{
    if(action == 0 || action == 17) {
        ...
        if (mp) {
            ...
            *(void **)((char *)info.dli_fbase + 0x1C3B8) = (void *)constget;
            *(void **)((char *)info.dli_fbase + 0x1C0C0) = (void *)IOConnectTrap4;
            
            x(NULL, tfp0, kernel_base, kaslr_shift, -1, -1);
            
            dlclose(h);
        } else {
            nukesb(1, 0, kernel_base, kaslr_shift);
            struct utsname uts;
            uname(&uts);
